...
try:
    # 'git_sync.py' nella stessa cartella
    from git_sync import sync_to_github
except ImportError:
    print("Attenzione: Non è stato possibile trovare il modulo 'git_sync.py'. La sincronizzazione GitHub non sarà eseguita.")
    # Se l'importazione fallisce, impostiamo la funzione a None
    sync_to_github = None
    
punti_selezionati = [] # Da P1 (indice 0) a P11 (indice 10)
...
def misura_immagine_interattiva(immagine_originale_in, filepath, scale_factor):
    ...
    '''1. Catturare l'input cv2.waitKey(1).
    2. Restituire (risultati, True) se viene premuto ESC (codice ASCII 27).
    3. Restituire (risultati, False) in tutti gli altri casi.'''
    ...
    print("Spostamento fine dell'ultimo punto: 'i' (Su), 'k' (Giù), 'j' (Sinistra), 'l' (Destra).")
    ...
    # --- ANELLO PRINCIPALE AGGIUNTO ---
    # Questo loop assicura che quando si torna indietro (es. da Fase 1 a 0)
    # il codice non termini, ma torni al blocco while corretto.
    while True: 
        ...
        while fase_corrente == 0:
            ...
            # Gestione Uscita Globale (ESC) - DEVE ESSERE IL PRIMO CONTROLLO
            if k == 27: 
                cv2.destroyAllWindows()
                return {"status": "Interrupted by ESC"}, True # Flag di interruzione globale
            elif k == ord('c'):
            ...
            # Input di Skip e Save (Q e S)
                ...
                return risultato_immagine, False
            ...
            # Il blocco 'key = cv2.waitKey(1) & 0xFF' è stato rimosso
            # perché ridondante e conflittuale con 'k = cv2.waitKey(50)'.
            
            # Se l'immagine viene aggiornata dalla callback del mouse, è necessario ridisegnare
            redraw_image()

            # Se la fase è cambiata (es. premendo 'c'), esci da questo loop
            if fase_corrente != 0:
                break
        ...
        while fase_corrente == 1:
            ...
            # Se la fase è cambiata (es. premendo 'c' o 'x'), esci da questo loop
            if fase_corrente != 1:
                break
        ...
        while fase_corrente == 3:
            ...
            # Se la fase è cambiata (es. premendo 'x'), esci da questo loop
            if fase_corrente != 3:
                break
            
    # --- Ritorno rimosso ---
    # Il 'return risultato_immagine' finale è stato rimosso.
    # L'uscita dalla funzione è ora gestita solo dai comandi
    # 'q' (skip), 's' (save), o ESC (interrupt) all'interno dei loop.
# ==============================================================================
...
def redraw_image():
    ...
    if len(punti_selezionati) >= 2:
        ...
        if len(punti_selezionati) >= 4:
            ...
            cv2.line(immagine_modificabile, p4_orig, p4_proj_orig.tolist(), (0, 0, 255), 1) # Rosso (Gradino)
            p1_orig, p2_orig, p4_orig = punti_selezionati[0], punti_selezionati[1], punti_selezionati[3]
            
            # Calcola il vettore direzione P1->P2
            v_dx = p2_orig[0] - p1_orig[0]
            v_dy = p2_orig[1] - p1_orig[1]
            
            # Creiamo un punto P4_virtuale_lontano a sinistra e a destra di P4
            # Per creare una retta che attraversi l'intera immagine (parallela a P1-P2)
            
            # Coefficiente di scala per estendere la linea (es. 20000 pixel)
            # Usiamo un valore grande per assicurarci che la linea attraversi la finestra di visualizzazione
            SCALA_ESTENSIONE = 50000 
            
            # Punto di partenza virtuale (molto a sinistra di P4)
            p_start_orig = (p4_orig[0] - v_dx * SCALA_ESTENSIONE, p4_orig[1] - v_dy * SCALA_ESTENSIONE)
            
            # Punto di arrivo virtuale (molto a destra di P4)
            p_end_orig = (p4_orig[0] + v_dx * SCALA_ESTENSIONE, p4_orig[1] + v_dy * SCALA_ESTENSIONE)
            
            # Trasforma i punti virtuali per il disegno
            #p_start_view = transform_point_to_view(p_start_orig)
            #p_end_view = transform_point_to_view(p_end_orig)
            
            # Disegna la linea di riferimento orizzontale (passante per P4, parallela a P1-P2)
            cv2.line(immagine_modificabile, p_start_orig, p_end_orig, (255, 255, 0), 1)

            cv2.line(immagine_modificabile, p3_orig, p4_orig, (255, 255, 0), 1)
...
def elabora_immagini_sequential(cartella_base):
    ...
    (Logica aggiornata per gestire l'uscita ESC e la sincronizzazione Git)
    """
    ...
    try:
        # Flag per interrompere il ciclo for
        break_global = False 
        
        for dirpath, _, filenames in os.walk(cartella_base):
            if break_global:
                break # Interrompe il ciclo for esterno
                
            # CICLO CORRETTO: Itera una sola volta sui filenames
            for filename in filenames:
                if break_global:
                    break # Interrompe il ciclo for interno
                ...
                if filename.lower().endswith(('.tif', '.tiff')):
                    ...
                    try:
                        ...
                        # 3. Misurazione interattiva: ORA RICEVIAMO DUE VALORI
                        risultati_immagine, break_global = misura_immagine_interattiva(
                            immagine_norm.copy(), percorso_completo, scale_factor
                        )
                        
                        # CONTROLLO USCTA GLOBALE (ESC)
                        if break_global:
                            cv2.destroyAllWindows()
                            print("\nInterruzione globale da utente (ESC). Fine elaborazione batch.")
                            break # Questo break esce dal ciclo dei filenames
                        
                        #cv2.destroyAllWindows()
                        
                        redraw_image() # Commentata per evitare errori se non definita
                        ...
                        if risultati_immagine.get("status") == "Skipped":
                            ...
                            # 4. SALVATAGGIO LOCALE E SINCRONIZZAZIONE GITHUB
                            ...
                            # ESEGUI SINCRONIZZAZIONE IMMEDIATA
                            if sync_to_github:
                                print("\nINIZIO SINCRONIZZAZIONE GITHUB...")
                                commit_msg = f"Aggiornamento misurazioni {filename}: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                                sync_to_github('riepilogo_misure.xml', commit_msg)
